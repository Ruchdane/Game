...
void verification(int t[TAILLE][TAILLE]);//IF TEST

void  SDL_stop(const char *message,SDL_Window* window,SDL_Renderer *renderer,SDL_Texture *texture);



int init0 (int t[TAILLE][TAILLE],int j);//Charger le niveau j s'il n'y a plus de niveau envoi 0

void translate (int t[TAILLE][TAILLE]);//transforme un tableau en donner l'ecrit en memoire voir 'perso'
...
int action (SDL_Event event,int t[TAILLE][TAILLE],int j);//modifie le tableau en fonction de l'action /j c'ast pour la direction de mario/
